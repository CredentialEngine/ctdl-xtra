"""Strict freeze-backed checks for course golden packs."""
...
def cross_check_pack(pack: Path) -> list[str]:
    ...
    for path in sorted(rec_dir.glob("*.json")):
        ...
        text_path = pack / rec["source"]["normalized_text_path"]
    ...
    errors.extend(check_xtra_course_goldens(pack))
    return errors


def check_xtra_course_goldens(pack: Path) -> list[str]:
