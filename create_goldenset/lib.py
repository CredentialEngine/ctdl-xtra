...
SCHEMA_SRC = ROOT / "record.schema.json"

# Windows working copy shown in File Explorer: This PC > OS (C:) > Code > golden_set
WINDOWS_GOLDEN_HOME = Path("C:/Code/golden_set")
...
def college_slug(url: str) -> str:
    """Hostname → folder slug. Kept here so run.py can pick --out before importing the generator."""
...
def configure(pack: Path) -> None:
    """Point the generator at this pack before that package is imported."""
...
def golden_set_home() -> Path:
    """Directory where new packs appear (File Explorer: C:\\Code\\golden_set)."""
    env = (os.environ.get("GOLDEN_SET_HOME") or "").strip()
    if env:
        return Path(env)
    for candidate in (
        WINDOWS_GOLDEN_HOME,
        Path(r"C:\Code\golden_set"),
        Path("/mnt/c/Code/golden_set"),
    ):
        if candidate.is_dir():
            return candidate
    return ROOT / "handoff"
...
def resolve_pack(out: str | None, urls: list[str]) -> Path:
    ...
    if out:
        pack = Path(out)
        return pack if pack.is_absolute() else home / pack
    if len(urls) == 1:
        return home / f"golden_set_{college_slug(urls[0])}_courses"
    return home / "golden_set_courses_mix"
...
def resolve_existing_pack(pack_arg: str) -> Path:
    ...
    if pack.is_absolute():
        return pack
    home_candidate = golden_set_home() / pack
    if home_candidate.exists():
        return home_candidate
    root_candidate = ROOT / pack
...
def write_pack_readme(pack: Path, *, seeds: list[str], errors: list[str]) -> Path:
    ...
    lines = [
        ...
        "",
        "Generated by `run.py`. **Not human-signed.**",
        ...
        "",
        "- Every `expected` value is copied from `snapshots/` / `normalized/`",
        ...
        "## Score (candidate)",
        "",
        "This folder only generates gold. Score extractor dumps in the full Nicolas-xTRA repo.",
        "",
    ]
