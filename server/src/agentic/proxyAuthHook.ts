...
export function readProxyAuthFromEnv(): ProxyAuthCredentials | undefined {
  ...
  try {
    ...
    console.error("[proxyAuthHook] PUPPETEER_PROXY_AUTH is not valid JSON");
    return undefined;
  }
  if (!parsed || typeof parsed !== "object") {
    console.error("[proxyAuthHook] PUPPETEER_PROXY_AUTH is not an object");
    return undefined;
  }
...
export async function installProxyAuthHook(
  ...
  const resolvedAuth = auth ?? readProxyAuthFromEnv();
  if (!resolvedAuth) {
    return;
  }

  await authenticatePage(page, resolvedAuth);
  ...
  browser.on("targetcreated", async (target) => {
    ...
    if (newPage) {
      await authenticatePage(newPage, resolvedAuth);
    }
  });
}

export async function applyProxyAuthToPage(
  page: Page,
  auth?: ProxyAuthCredentials
): Promise<void> {
  const resolvedAuth = auth ?? readProxyAuthFromEnv();
