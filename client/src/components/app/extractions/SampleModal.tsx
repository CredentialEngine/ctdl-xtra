...
export default function SampleModal({
  ...
  const [sortBy, setSortBy] = useState<
    "random" | "most_expensive" | "most_data_items"
  >("random");
  ...
  const [appliedFilters, setAppliedFilters] = useState<{
    ...
    statuses: PageStatus[];
    sortBy: "random" | "most_expensive" | "most_data_items";
    applyKey: number;
  } | null>(null);
        ...
        <DialogHeader className="px-6 pt-6 pb-4 shrink-0">
          ...
          {(totalInputTokens != null || totalOutputTokens != null) && (
            <p className="text-sm text-muted-foreground mt-1">
              In: {formatMillions(totalInputTokens ?? 0)} · Out:{" "}
              {formatMillions(totalOutputTokens ?? 0)}
            </p>
          )}
        </DialogHeader>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4 shrink-0">
            ...
            <div>
              ...
              <Select
                value={sortBy}
                onValueChange={(
                  v: "random" | "most_expensive" | "most_data_items"
                ) => setSortBy(v)}
              >
                <SelectTrigger>
                  <SelectValue />
