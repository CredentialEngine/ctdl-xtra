              ...
              <Checkbox
                ...
                onCheckedChange={(checked) => {
                  if (checked) {
                    ...
                    form.setValue(`${path}.clickSelector` as Path<any>, "body", { shouldValidate: false });
                    const clickOptionsPath = `${path}.clickOptions` as Path<any>;
                    const currentClickOptions = form.getValues(clickOptionsPath);
                    if (!currentClickOptions?.limit) {
                      form.setValue(clickOptionsPath as Path<any>, { limit: 300 }, { shouldValidate: false });
                    }
                  } else {
                    // When unchecked, clear clickSelector and clickOptions
                    form.setValue(`${path}.clickSelector` as Path<any>, undefined, { shouldValidate: false });
                    form.setValue(`${path}.clickOptions` as Path<any>, undefined, { shouldValidate: false });
                  }
                }}
              />
            </FormControl>
