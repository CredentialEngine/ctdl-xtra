from __future__ import annotations

from urllib.parse import urlparse

from common.local_storage_provider import LocalStorageProvider
from common.storage_configuration import AzureStorageConfiguration
from common.storage_uri import (
    parse_azure_storage_uri,
    parse_file_storage_uri,
    parse_local_path_if_supported,
)


def create_storage_provider(
    *,
    source_uri: str,
    batch_size: int,
    read_concurrency: int,
    azure_storage_connection_string: str | None = None,
):
    parsed = urlparse(source_uri)
    scheme = parsed.scheme.lower()

    if scheme == "azure":
        if not azure_storage_connection_string:
            raise ValueError("Azure storage connection string is required")
        from common.azure_storage_provider import AzureBlobStorageProvider

        azure_uri = parse_azure_storage_uri(source_uri)
        return AzureBlobStorageProvider(
            container_name=azure_uri.container_name,
            prefix_path=azure_uri.prefix_path,
            batch_size=batch_size,
            read_concurrency=read_concurrency,
            azure_configuration=AzureStorageConfiguration(
                connection_string=azure_storage_connection_string
            ),
        )

    if scheme == "file":
        file_uri = parse_file_storage_uri(source_uri)
        return LocalStorageProvider(
            root_path=file_uri.local_path,
            batch_size=batch_size,
            read_concurrency=read_concurrency,
        )

    local_path = parse_local_path_if_supported(source_uri)
    if local_path:
        return LocalStorageProvider(
            root_path=local_path,
            batch_size=batch_size,
            read_concurrency=read_concurrency,
        )

    raise ValueError(f"Unsupported storage URI: {source_uri}")
