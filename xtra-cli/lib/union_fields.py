...
Live (Playwright, slow):
  ...
  python3 xtra-cli/xtra_cli.py field union --url-file xtra-cli/lib/union_fields_urls.txt --limit 5

From an existing pack (no crawl):

  python3 xtra-cli/xtra_cli.py field union --from-pack my_pack
"""
...
def run_one(url: str, *, limit: int) -> dict:
    ...
    cmd = [
        sys.executable,
        str(CLI_ROOT / "extraction" / "run.py"),
        "--url",
        url,
        "--limit",
        str(limit),
