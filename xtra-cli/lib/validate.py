...
def validate_record(rec: dict, *, pack: Path) -> list[str]:
    ...
    snap = pack_join(pack, rec["source"]["snapshot_path"])
    norm = pack_join(pack, rec["source"]["normalized_text_path"])
    if not snap.is_file():
...
def validate_pack(pack: Path, *, strict_promotion: bool = False) -> list[str]:
    ...
    mode = env_get("XTRA_MODE")
    types = {r["entity_type"] for r in records}
    college_pack = mode == "college" or types <= {"Course"}
    if college_pack:
        if n < 1:
