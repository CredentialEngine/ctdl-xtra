...
_EXAMPLE_CATALOG = "https://catalog.brookdalecc.edu"


class HarvestError(RuntimeError):
    """Crawl cannot open a seed URL (DNS, connection, or Playwright navigation)."""


def navigation_failure(url: str, exc: BaseException) -> str:
    """One-line message for a failed page.goto. Do not dump the Playwright traceback."""
    detail = str(exc).splitlines()[0].strip()
    if "ERR_NAME_NOT_RESOLVED" in str(exc):
        return (
            f"cannot open {url}: host not found (net::ERR_NAME_NOT_RESOLVED). "
            f"Pass a live catalog home or course URL (for example {_EXAMPLE_CATALOG})."
        )
    return f"cannot open {url}: {detail}"


def iter_json_codes(obj, acc: list[str]) -> None:
    if isinstance(obj, dict):
        # Skip Coursedog document `id` (random mixed-case tokens). Use printed codes.
        for key in ("code", "courseId", "displayName"):
            val = obj.get(key)
