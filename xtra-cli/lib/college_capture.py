"""Run a college-URL → candidate Course pack."""
    ...
    for seed in seeds:
        ...
        try:
            report = harvest_with_playwright(
                seed, want=max(want_each * 4, want_each), fetch_all=fetch_all
            )
...
def run_college_capture(
    ...
    *,
    ...
    return {"records": len(all_slots), "errors": problems, "pack": PACK.name}
